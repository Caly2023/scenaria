...
export function useScriptDoctor({
  ...
  const executeToolCall = async (call: any, retryAttempt: number = 0): Promise<any> => {
    if (!currentProject) return { success: false, error: 'No active project', error_code: 0 };
    ...
    try {
      switch (name) {
        ...
        case 'get_stage_structure': {
          const { stage_id } = args;
          telemetryService.setStatus('get_stage_structure', '🧠', `Mapping Primitive IDs for ${stage_id}...`);
        ...
        case 'research_context': {
          ...
          telemetryService.setStatus('research_context', '🔍', `Retrieving ${stageName} with primitive IDs...`);
          
          const enrichWithIds = (items: any[]) => items.map(item => ({
          ...
          if (stageName === 'Character Bible') return { success: true, data: enrichWithIds(characters) };
          if (stageName === 'Location Bible') return { success: true, data: enrichWithIds(locations) };
          if (stageName === 'Step Outline') return { success: true, data: enrichWithIds(sequences) };
        ...
        case 'fetch_character_details': {
          ...
          if (!char) return { success: false, error: `Character with primitive_id '${characterId}' not found`, error_code: 404 };
        ...
        case 'search_project_content': {
          ...
          const results = {
            characters: characters.filter(c => c.name.toLowerCase().includes(q) || c.description.toLowerCase().includes(q)).map(c => ({ primitive_id: c.id, ...c })),
            locations: locations.filter(l => l.name.toLowerCase().includes(q) || l.description.toLowerCase().includes(q)).map(l => ({ primitive_id: l.id, ...l })),
        ...
        case 'propose_patch': {
          ...
          telemetryService.setStatus('propose_patch', '📡', `Sending update to Firebase (ID: ${id})...`, id);
          setRefiningBlockId(id);
          
          if (!updates || (Object.keys(updates).length === 0)) {
            setRefiningBlockId(null);
            return { success: false, error: 'Empty content update', error_code: 400 };
          }

          const subcollection = subcollectionMap[stage];

          if (subcollection) {
            const docRef = doc(db, 'projects', currentProject.id, subcollection, id);
        ...
        case 'execute_multi_stage_fix': {
          const { fixes } = args;
          if (!fixes || !Array.isArray(fixes)) return { success: false, error: 'Invalid fixes array', error_code: 400 };

          const results: Array<{ primitive_id: string; success: boolean; error?: string }> = [];
          
          for (const fix of fixes) {
            const { id, stage, updates } = fix;
            telemetryService.setStatus('multi_stage_fix', '📡', `Sending update to Firebase (ID: ${id})...`, id);
            
            try {
              const subcollection = subcollectionMap[stage];
              if (subcollection) {
                await updateDoc(doc(db, 'projects', currentProject.id, subcollection, id), {
                  ...updates,
                  updatedAt: serverTimestamp()
                });
                telemetryService.invalidateStage(stage);
              ...
              const classification = telemetryService.classifyFirebaseError(fixError);
              results.push({ primitive_id: id, success: false, error: classification.message });
            }
          }
        ...
        case 'sync_metadata': {
          ...
          telemetryService.setStatus('Confirmed', '✅', 'Project metadata synced.');
          addToast(t('common.metadataSynced', { defaultValue: 'Project metadata synced' }), 'success');
          return { success: true };
        }
        ...
        case 'add_primitive': {
          ...
          if (subcollection) {
            const newDocRef = await addDoc(collection(db, 'projects', currentProject.id, subcollection), {
              ...primitive,
              order: position ?? 0,
            ...
            return { success: false, error: `Adding primitives not supported for stage: ${stage}`, error_code: 400 };
          }
        }
        ...
        case 'delete_primitive': {
          ...
          if (subcollection) {
            ...
            return { success: false, error: `Deleting primitives not supported for stage: ${stage}`, error_code: 400 };
          }
        }

        case 'restructure_stage': {
          const { stage, primitives } = args;
          if (!primitives || !Array.isArray(primitives)) return { success: false, error: 'Invalid primitives array', error_code: 400 };
          ...
          if (subcollection) {
            ...
            for (let i = 0; i < primitives.length; i++) {
              ...
              const newRef = await addDoc(collection(db, 'projects', currentProject.id, subcollection), {
                ...p,
                order: i,
                projectId: currentProject.id,
                createdAt: serverTimestamp()
            ...
            telemetryService.invalidateStage(stage);
            addToast(t('common.stageRestructured', { defaultValue: 'Stage restructured successfully' }), 'success');
            return { success: true, new_primitive_ids: newIds };
          } else {
            return { success: false, error: `Restructuring not supported for stage: ${stage}`, error_code: 400 };
          }
        }
        ...
        default:
          return { success: false, error: `Unknown tool: ${name}`, error_code: 400 };
      }
    } catch (error) {
      ...
      telemetryService.setStatus('Error', '❌', `${classification.type}: ${classification.message}`, targetId);
      
      if (failureCount >= 2 && retryAttempt < 2) {
        telemetryService.setStatus('Recovery', '🔄', 'Double failure detected. Full ID-Map resync...');
      ...
      if (classification.type === 'PERMISSION_DENIED') {
        addToast(`🔒 Security block on ${name}: ${classification.message}`, 'error');
      }
  ...
  const handleDoctorMessage = async (content: string) => {
    ...
    try {
      ...
      if (parsed?.type === 'apply_suggestion') {
        const { msgId, action } = parsed as { type: string; msgId?: string; action?: string };
        ...
        if (msgId) {
          ...
          if (referencedMsg) {
            ...
            if (Array.isArray(referencedMsg.suggested_actions) && referencedMsg.suggested_actions.length > 0) {
              detailLines.push(`Suggested actions from the message: ${referencedMsg.suggested_actions.map((a: string) => `"${a}"`).join(', ')}.`);
            }

            // 2. Surface any embedded patch / structured content blocks from the content string
            const rawContent: string = typeof referencedMsg.content === 'string' ? referencedMsg.content : JSON.stringify(referencedMsg.content ?? '');

            // Extract fenced JSON/patch blocks if present
            const patchBlockMatch = rawContent.match(/```(?:json)?\s*(\{[\s\S]*?\})\s*```/);
            if (patchBlockMatch?.[1]) {
              detailLines.push(`Patch details extracted from the message:\n${patchBlockMatch[1]}`);
            } else if (rawContent.length > 0 && rawContent.length <= 2000) {
              // Include a trimmed version of the full response as context
              detailLines.push(`Full context from the message:\n${rawContent.substring(0, 2000)}`);
            }
          }
        }

        // Build a precise, tool-aware instruction for the agentic loop
        const actionDescription = action ? `"${action}"` : 'the suggestion described in the referenced message';
        const extraContext = detailLines.length > 0 ? `\n\nAdditional context:\n${detailLines.join('\n')}` : '';
    ...
    const userMsg = { id: Date.now().toString(), role: 'user', content: resolvedContent, timestamp: Date.now() };
    ...
    const complexKeywords = [
      'generate', 'break down', 'rewrite', 'fix', 'restructure', 'create', 'write', 'develop', 'analyze all', 'full audit',
      'génère', 'générer', 'réécrire', 'réécris', 'corriger', 'corrige', 'restructurer', 'créer', 'crée', 'écrire', 'écris',
      'développer', 'développe', 'analyser', 'analyse tout', 'audit complet', 'refaire', 'refais',
      'modifier', 'modifie', 'changer', 'change', 'supprimer', 'supprime', 'ajouter', 'ajoute',
      'delete', 'remove', 'add', 'update', 'modify', 'apply'
    ];
    const isComplex = complexKeywords.some(kw => contentLower.includes(kw));
    
    const actionKeywords = [
      'supprimer', 'supprime', 'delete', 'remove', 'ajouter', 'ajoute', 'add',
      'modifier', 'modifie', 'modify', 'change', 'update', 'apply', 'fix', 'corriger', 'corrige'
    ];
    const isActionRequest = actionKeywords.some(kw => contentLower.includes(kw));
    ...
    try {
      ...
      if (isQuotaExhausted) {
        telemetryService.setStatus('Degraded Mode', '💬', 'Gemini 3 quota exhausted. Using chat-only mode...');
        ...
        let degradedText = degradedResult?.candidates?.[0]?.content?.parts?.find((p: any) => p.text)?.text
          ?? degradedResult?.text
          ?? '';
        ...
        try {
          ...
          degradedParsed = {
            status: '💬 Chat Mode',
            response: degradedText || "I'm in chat-only mode. How can I help you?",
            suggested_actions: ['Ask a question', 'Continue']
          };
        }

        if (!degradedParsed.response || degradedParsed.response.trim() === '') {
          degradedParsed.response = "I'm in chat-only mode due to quota limits. Ask me anything about your project — I'll do my best to help!";
        }

        telemetryService.setStatus('Complete', '💬', degradedParsed.status || 'Chat Mode');
        ...
        const botMsg = {
          ...
          suggested_actions: degradedParsed.suggested_actions || ['Continue', 'Ask something else'],
      ...
      for (let iteration = 0; iteration < MAX_ITERATIONS; iteration++) {
        const result = await geminiService.scriptDoctorAgent(conversationHistory, context, activeStage, complexity, idMapContext);
        ...
        if (functionCallParts.length === 0) {
          finalResponse = textParts.map((p: any) => p.text).join('') || result?.text || '';
          break;
        }
        ...
        for (const part of functionCallParts) {
          ...
          const toolStatusMap: Record<string, [string, string]> = {
            ...
            'get_stage_structure': ['🧠', `Mapping Primitive IDs for ${call.args?.stage_id || 'stage'}...`],
            'fetch_character_details': ['🔍', 'Analyzing character profiles...'],
            'search_project_content': ['🔍', 'Searching narrative threads...'],
            'propose_patch': ['📡', `Sending update to Firebase (ID: ${call.args?.id || '...'})...`],
            ...
            'delete_primitive': ['🗑️', `Removing element (ID: ${call.args?.id || '...'})...`],
            'fetch_project_state': ['🧠', 'Loading full project state + ID-Map...'],
            'update_stage_insight': ['📊', `Updating stage insight...`],
            'restructure_stage': ['🔄', `Restructuring stage...`],
          };
          const [emoji, detail] = toolStatusMap[call.name] || ['⚡', `Executing ${call.name}...`];
          ...
          if (toolResult.success) {
            telemetryService.setStatus('Confirmed', '✅', `Confirmation received. Syncing UI...`, toolResult.primitive_id);
            if (['propose_patch', 'execute_multi_stage_fix', 'add_primitive', 'delete_primitive', 'restructure_stage'].includes(call.name)) {
              contentChanged = true;
            }
            ...
            const classification = telemetryService.classifyFirebaseError({ code: toolResult.error_code, message: toolResult.error });
            
            if (classification.action === 'RESYNC_AND_RETRY' && call.args?.id) {
              telemetryService.setStatus('Resync', '🔄', 'ID not found. Re-syncing Primitive IDs...');
              await contextAssembler.hydrateFullIdMap(currentProject.id);
              ...
              if (retryResult.success && ['propose_patch', 'execute_multi_stage_fix', 'add_primitive', 'delete_primitive', 'restructure_stage'].includes(call.name)) {
                contentChanged = true;
              }
              continue;
            }
        ...
        telemetryService.setStatus('Continuing', '🔄', `Processing step ${iteration + 2}...`);
      ...
      try {
        ...
        if (jsonMatch) {
          try {
            ...
            parsedResponse = {
              status: "✅ Done",
              response: finalResponse || "I received your message but couldn't format the response properly. Could you try again?",
      ...
      if (!parsedResponse.response || parsedResponse.response.trim() === '') {
        parsedResponse.response = "I processed your request but the response was empty. Could you rephrase?";
        parsedResponse.suggested_actions = ["Try again", "Ask differently"];
      }

      telemetryService.setStatus('Complete', '✅', parsedResponse.status || 'Done');
