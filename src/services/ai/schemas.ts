...
export const SCRIPT_DOCTOR_FUNCTION_DECLARATIONS = [
  ...
  { name: 'update_stage_insight',    description: 'Provides a professional analysis (insight) of the current step state and readiness.',                                               parameters: { type: 'OBJECT', properties: { stage: { type: 'STRING' }, insight: { type: 'OBJECT', properties: { content: { type: 'STRING' }, isReady: { type: 'BOOLEAN' }, suggestions: { type: 'ARRAY', items: { type: 'STRING' } }, suggestedPrompt: { type: 'STRING' }, score: { type: 'NUMBER' } } } }, required: ['stage', 'insight'] } },
