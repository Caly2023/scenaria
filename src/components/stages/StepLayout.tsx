  ...
  return (
    <div className="w-full h-auto flex-1 flex flex-col space-y-12 md:space-y-16 pb-4 md:pb-40">
      ...
      <ConfirmationModal
        ...
        title={isReady ? 'Continuer ?' : 'Passer à l\'étape suivante ?'}
        description={
          <>
            {!isReady && <span className="text-amber-500/80 block mb-2">Note : Cette étape n'a pas encore été validée.</span>}
            Cette action verrouille l'étape {t(`stages.${stageName}.label`, { defaultValue: stageName })} et génère la prochaine.
          </>
        }
        confirmLabel={isReady ? 'Continuer' : 'Continuer quand même'}
      />
    </div>
  );
}
