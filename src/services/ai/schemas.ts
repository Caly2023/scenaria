...
export const SCRIPT_DOCTOR_FUNCTION_DECLARATIONS = [
  ...
  { name: 'update_primitives',       description: 'Submit modifications for multiple primitives at once. If updating just one, pass an array with a single element. IDs MUST be valid primitive_ids from the ID-MAP.', parameters: { type: 'OBJECT', properties: { stage: { type: 'STRING' }, updates: { type: 'ARRAY', items: { type: 'OBJECT', properties: { id: { type: 'STRING' }, fields: { type: 'OBJECT', properties: { title: { type: 'STRING' }, content: { type: 'STRING' }, primitiveType: { type: 'STRING', description: 'CRITICAL: Must specify type, e.g. "character", "location", "scene", "beat"' } } } }, required: ['id', 'fields'] } } }, required: ['stage', 'updates'] } },
  ...
  { name: 'add_primitives',          description: 'Adds new structural elements (primitives) to a production stage. Takes an array of primitives. If adding one, pass an array with one element.', parameters: { type: 'OBJECT', properties: { stage: { type: 'STRING' }, primitives: { type: 'ARRAY', items: { type: 'OBJECT', properties: { title: { type: 'STRING' }, content: { type: 'STRING' }, primitiveType: { type: 'STRING', description: 'CRITICAL: MUST be provided (e.g., "character", "location", "scene", "beat")' } }, required: ['title', 'content', 'primitiveType'] } } }, required: ['stage', 'primitives'] } },
  { name: 'delete_primitives',       description: 'Removes multiple specific elements from production using their primitive_ids. Takes an array of ids. If deleting one, pass an array with one id.', parameters: { type: 'OBJECT', properties: { ids: { type: 'ARRAY', items: { type: 'STRING' } }, stage: { type: 'STRING' } }, required: ['ids', 'stage'] } },
  ...
  { name: 'update_stage_insight',    description: 'Provides a professional analysis (insight) of the current step state and readiness. Updates both the global project analysis and the local AI Insight primitive.', parameters: { type: 'OBJECT', properties: { stage: { type: 'STRING', description: 'The stage ID to analyze' }, insight: { type: 'OBJECT', properties: { evaluation: { type: 'STRING', description: 'Professional narrative evaluation (markdown). Use this for the main analysis text.' }, isReady: { type: 'BOOLEAN', description: 'CRITICAL: Set to true if the stage passes quality gates and is ready for the next step.' }, issues: { type: 'ARRAY', items: { type: 'STRING' }, description: 'Identified weaknesses, gaps, or contradictions.' }, recommendations: { type: 'ARRAY', items: { type: 'STRING' }, description: 'Actionable improvement suggestions.' }, suggestedPrompt: { type: 'STRING', description: 'A prompt the user can use to fix the identified issues.' }, score: { type: 'NUMBER', description: 'Quality score from 0 to 100.' } }, required: ['evaluation', 'isReady'] } }, required: ['stage', 'insight'] } },
